...
class HedTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        data = data_dict.get(self.data_key).cpu().numpy()
        seg = data_dict.get(self.label_key)

        if np.random.uniform() <= self.p_per_sample:
            data = (data * 255).clip(0,255).astype(np.uint8) # clip prevents overflow after uint8 conversion. 256 needed for hed
            self.hed_transform.randomize()
            data = self.hed_transform.transform(data)
            data = (data / 255).astype(np.float32) # back to float
        data_dict[self.data_key] = torch.Tensor(data).to(seg.device)
        return data_dict
